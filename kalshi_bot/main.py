...
# Load backtest-calibrated per-city p75 peak times (minutes since midnight).
# Generated by scripts/backtest_peak_hour.py — falls back to global env var if absent.
# p75 = time by which 75% of historical days have seen their daily high (used for
# weather-filter gates in main.py; band_arb is_locked also uses p75 via strike_arb).
