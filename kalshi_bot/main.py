...
# Load backtest-calibrated per-city p90 peak times (minutes since midnight).
# Generated by scripts/backtest_peak_hour.py — falls back to global env var if absent.
_PEAK_P90_MINUTES: dict[str, dict[int, int]] = {}
...
if _p90_path.exists():
    ...
    if _spec and _spec.loader:
        ...
        _PEAK_P90_MINUTES = getattr(_p90_mod, "P75_MINUTES", {})
            ...
            if opp.implied_outcome == "YES":
                ...
                if opp.source in _OBS_CONFIRMED and opp.direction in ("under", "between"):
                    ...
                    if city_tz is not None:
                        local_dt = now.astimezone(city_tz)
                        local_mins = local_dt.hour * 60 + local_dt.minute
                        _p90 = _PEAK_P90_MINUTES.get(opp.metric, {}).get(local_dt.month)
                        min_mins = _p90 if _p90 is not None else (
