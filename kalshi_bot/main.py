...
from .weather_filter import (
    ...
    _apply_forecast_consensus,
    HRRR_MAX_SPREAD_F,
    TEMP_OBSERVED_MAX_HOURS,
)
...
# Weather filter config (TEMP_OBSERVED_*, HRRR_MAX_SPREAD_F, FORECAST_CORROBORATION_MIN,
# OBS_CONSENSUS_MIN, NOAA_OBS_*, MORNING_OBS_GAP_F, KXLOWT_*) moved to weather_filter.py.
...
def _passes_liquidity(ticker_detail: dict, ticker: str) -> bool:
    detail = ticker_detail.get(ticker)
    if not detail:
...
async def _poll(
    ...
    if missing_tickers:
        ...
        detail_results = await asyncio.gather(
            *[_fetch_detail(session, _detail_sem, t) for t in missing_tickers],
            return_exceptions=True,
        )
    ...
    # ---- liquidity filter --------------------------------------------------
    text_opps    = [o for o in text_opps    if _passes_liquidity(ticker_detail, o.market_ticker)]
    numeric_opps = [o for o in numeric_opps if _passes_liquidity(ticker_detail, o.market_ticker)]
    poly_opps    = [o for o in poly_opps    if _passes_liquidity(ticker_detail, o.kalshi_ticker)]
    ...
    if TRADE_DRY_RUN and ledger is not None and obs_values:
        try:
            ...
            for _row_id, _row_ticker, _row_note in _lowt_yes_rows:
                ...
                if _metric is None or _strike_lo is None:
                    continue
                _cur_min = obs_values.get(_metric)
                if _cur_min is None:
                    continue
                if _cur_min < _strike_lo - 0.5:
                    logging.warning(
                        "[lowt-floor-breach] trade #%d %s: obs_min=%.1f°F < floor=%.1f°F"
                        " — low has left the band, queuing force-exit.",
                        _row_id, _row_ticker, _cur_min, _strike_lo,
                    )
                    ledger.request_force_exit(
                        _row_ticker, f"metar_lowt_floor_breach@{_cur_min:.1f}F"
                    )
        except Exception as _exc:
            logging.debug("Fast loop: KXLOWT YES floor-breach check failed: %s", _exc)
