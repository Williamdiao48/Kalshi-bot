...
async def main(
    ...
    if emit_dict:
        ...
        lines = [
            ...
            "# p75/p90 thresholds: local time by which 75%/90% of daily highs have been observed",
            "",
        ...
        lines.append("}")
        lines.append("")
        lines.append("P90_MINUTES: dict[str, dict[int, int]] = {")
        for metric in sorted(all_stats):
            monthly = all_stats[metric]
            inner = ", ".join(
                f"{m}: {monthly[m]['p90']}"
        ...
        for pct, key in [("P75", "p75"), ("P90", "p90")]:
            print(f"\n=== {pct} peak times by city and month (HH:MM local) ===")
