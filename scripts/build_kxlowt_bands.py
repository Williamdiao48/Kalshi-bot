"""Fetch all settled KXLOWT* band (between) markets from Kalshi and write to
data/kxlowt_bands.csv.

Pulls historical settled markets for every low-temperature series, parses
direction/strike using market_parser.parse_market(), and merges into the CSV.
Safe to re-run: deduplicates by ticker.

Run:
  venv/bin/python scripts/build_kxlowt_bands.py
  venv/bin/python scripts/build_kxlowt_bands.py --dry-run   # print rows, don't write
"""
...
OUT_CSV = Path(__file__).parent.parent / "data" / "kxlowt_bands.csv"

CSV_FIELDS = ["ticker", "metric", "date", "direction", "strike_lo", "strike_hi", "band_width", "result"]

# All series that resolve to a temp_low_* metric
LOW_SERIES: list[str] = sorted(
    {prefix for prefix, metric in TICKER_TO_METRIC.items() if metric.startswith("temp_low_")}
)
...
async def _fetch_series(
    ...
    *,
    ...
    while True:
        ...
        for m in markets:
            pm = parse_market(m)
            if pm is None or pm.direction != "between":
                continue
...
async def main(dry_run: bool) -> None:
    ...
    print(f"Series to fetch: {len(LOW_SERIES)}  ({', '.join(LOW_SERIES)})\n")
    ...
    async with aiohttp.ClientSession() as session:
        for series in LOW_SERIES:
            ...
            print(f"  {series:<18} {len(rows):>5} settled  {len(fresh):>5} new  ({elapsed:.1f}s)")
